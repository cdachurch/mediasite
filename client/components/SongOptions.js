...
class SongOptions extends React.Component {
  updateSongParameters = (songKey, textSize, vocalistMode) => {
    this.props.router.push(
      `/song/${this.props.songId}/print?songKey=${songKey.replace(
        /#/g,
        "%23"
      )}&textSize=${textSize}&vocalistMode=${vocalistMode}`
    );
  };

  updateChosenSongKey = event => {
    const newKey = event.target.value;
    this.updateSongParameters(
      newKey,
      this.props.textSize,
      this.vocalistMode.checked
    );
  };
  ...
  render() {
    ...
    return (
      <div
        className="row hide-on-small-only no-print"
        style={{ position: "fixed", right: 0 }}
        ref={comp => (this.wholeComp = comp)}
      >
          ...
          <div className="card-panel">
            <p>
              ...
              <i
                ...
                onClick={() => (this.wholeComp.style.display = "none")}
                style={{ float: "right" }}
              >
                close
              </i>
  ...
  stepKey = amount => {
    ...
    if (amount === -1) {
      if (currentKeyIndex === 0) {
        this.updateSongParameters(
          MUSICAL_KEYS[MUSICAL_KEYS.length + amount],
          this.props.textSize
        );
      } else {
        this.updateSongParameters(
          MUSICAL_KEYS[possibleNewKeyIndex],
          this.props.textSize
        );
      }
    } else {
