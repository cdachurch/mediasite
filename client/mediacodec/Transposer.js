class Transposer {
  ...
  // For UI, mostly.
  static allKeys = unique(
    Transposer.sharpKeySignatures.concat(Transposer.flatKeySignatures).sort()
  );
  ...
  transposeNote(note) {
    ...
    if (note.indexOf("/") >= 0) {
      ...
      return note
        .split("/")
        .map(this.transposeNote.bind(this))
        .join("/");
    }
    ...
    if (isNoteFound === true) {
      const newNoteIndex = Transposer.shiftNote(
        noteIndex,
        this.getNoteShiftVal()
      );
  ...
  getNoteShiftVal() {
    ...
    this.noteShiftVal =
      Transposer.getNoteIndex(this.transposeKey) -
      Transposer.getNoteIndex(this.songKey);
    return this.noteShiftVal;
  }
